...
# Research Design
 ...
 - Then examine the associations between ATSR and student outcomes
   - we maximize the estimation precision by accounting for school fixed-effects, student baseline scores, and a rich set of student, teacher, homeroom characteristics. 
...
#### Key to our identification strategy: random assignment of teachers to students in China
 ...
 - In practice, schools create either random or stratified homerooms of students and teacher groups upon students’ entry to school, then randomly assign a teacher group to each student homeroom
...
We first identify a valid IV that meets two critical assumptions of IV estimation:
...
We conclude that teacher female is a valid instrumental variable.
...
To answer RQ3, we use teacher female as IV to estimate the local average treatment effects of ATSR on student outcomes in a two-stage least squares approach, where we
...
In research design
 ...
 - Our estimates are "localized" around the IV/teacher female
   - future research using different instrumental variable may have different estimates
