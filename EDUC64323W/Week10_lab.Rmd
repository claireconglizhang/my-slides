...
```{r read in data}
df0 <- read.csv("https://daviddliebowitz.github.io/EDUC643_23W/data/seda.csv") 
```
...
```{r}
df <- df %>% 
  mutate(grade = factor(grade, levels = c(5, 1, 2, 3, 4)))
...
margins::margins(fit1,
  ...
  geom_ribbon(aes(ymin = fitted-1.96*se.fitted, ymax = fitted+1.96*se.fitted), alpha=0.3, linetype=0) +
